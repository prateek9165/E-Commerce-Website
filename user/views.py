...
def login_form(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            #*** Multi Language
            # store language in session under the configured language cookie name
            session_key = getattr(settings, 'LANGUAGE_COOKIE_NAME', 'django_language')
            request.session[session_key] = userprofile.language.code
            # UserProfile.currency is a CharField now (currency code), store directly
            request.session['currency'] = userprofile.currency
